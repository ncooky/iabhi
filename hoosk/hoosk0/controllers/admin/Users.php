...
class Users extends CI_Controller {
	...
	public function index()
	{
        ...
        $this->data['usersNA'] = $this->Hoosk_model->getUsersNonAdm($result_per_page, $this->uri->segment(3));
	...
	public function editUser()
	{
        ...
        $this->data['userRole'] = $this->Hoosk_model->getUserRole();
        $this->data['userRoleNA'] = $this->Hoosk_model->getUserRoleNA();
		//Load the view
		$this->data['header'] = $this->load->view('admin/header', $this->data, true);
		$this->data['footer'] = $this->load->view('admin/footer', '', true);
	...
	public function edited()
	{
		...
		$this->load->library('form_validation');
		//Set validation rules
		$this->form_validation->set_rules('email', 'email address', 'trim|required|valid_email|is_unique[user.email.userID.'.$this->uri->segment(4).']');
		$this->form_validation->set_rules('password', 'password', 'trim|required|min_length[4]|max_length[32]');
		$this->form_validation->set_rules('con_password', 'confirm password','trim|required|matches[password]');
...
//			$config['charset'] = 'iso-8859-1';
            ...
            $config['smtp_host'] = "ssl://beta.iabhi.or.id";
            $config['smtp_port'] = "465";
            $config['smtp_user'] = "info@beta.iabhi.or.id"; 
            $config['smtp_pass'] = "14bh1";
			...
			$this->email->from('password@'.EMAIL_URL, $this->lang->line('email_reset_subject'));
			$this->email->to($email);
			
			$this->email->subject($this->lang->line('email_reset_subject'));
			$this->email->message($this->lang->line('email_reset_message')."\r\n".BASE_URL.'/resetpassword/'.$rs );
			
			$this->email->send();
            
            if ( ! $this->email->send())
            {
                echo "Pesan tidak sampai";
            }
            $this->data['header'] = $this->load->view('admin/headerlog', $this->data, true);
			$this->data['footer'] = $this->load->view('admin/footer', '', true);
