   ...
   <?php $i=0; 
     ...
     <div class="control-group" id="pic_element<?php echo $i; ?>">								
        ...
        <div class="controls">
            ...
            <p><?php echo $this->lang->line('pages_slider_titletext'); ?></p>
            <input type="text" id="title<?php echo $i; ?>" name="title<?php echo $i; ?>" <?php if ($s['slideTitle'] != "") { echo 'value="'.$s['slideTitle'].'"'; } else { echo  ''; } ?> />           
            <p><?php echo $this->lang->line('pages_slider_message'); ?></p>
            
            <!-- <input id="text<?php echo $i; ?>" name="text<?php echo $i; ?>" <?php if ($s['slideText'] != "") { echo 'value="'.$s['slideText'].'"'; } else { echo  ''; } ?> /> -->
            <?php
            if ($s['slideText'] != "") {
                 $sVal = $s['slideText']; 
                } else {
...
function formSubmit(){
	...
	for(i=1;i<=total_pics;i++)
	{
		if(document.getElementById('slide' + i))
		{
			...
			data_to_post += '{' + slide + '|' + link + '|' + alt + '|' + title + '|' + text /*+ '}'*/;
		}
	}
...
<script type="text/javascript">
	...
	function addNewPic()
	{
		...
		new_title_span = document.createElement('p');
		new_title_span.innerHTML =  '<?php echo $this->lang->line('pages_slider_titletext'); ?>';
		
		new_title = document.createElement('input');
		new_title.type = 'text';
		new_title.id = 'title' + total_pics;
		new_title.name = 'title' + total_pics;        

		new_text_span = document.createElement('p');
		new_text_span.innerHTML =  '<?php echo $this->lang->line('pages_slider_message'); ?>';
		
		new_text = document.createElement('textarea');
		new_text.maxLength = '5000';
        new_text.cols = '10';
        new_text.rows = '5';
		new_text.id = 'text' + total_pics;
		new_text.name = 'text' + total_pics;          
		...
		new_div2.appendChild(new_title_span);
		new_div2.appendChild(new_title);
		new_div2.appendChild(new_text_span);
		new_div2.appendChild(new_text);                
