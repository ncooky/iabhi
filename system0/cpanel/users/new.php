<?php
  ...
  if ((isset($_POST["accion"])) && ($_POST["accion"] == "insert")) {
    ...
    $existe = bfRecord(sprintf("SELECT username FROM %susers WHERE username='%s';", $tables_preffix, $username));
    if ($existe) { 
	//The user ya existe
      $Session["users"]["message"] .= ', ' . $lang['cpanel']['users']['username_exists']; 	
	  $no = 1;
    }
    ...
    if (!eregi("^([_a-z0-9-]+)(\.[_a-z0-9-]+)*$", $data['username'])) { 
	//The name de user esta mal
      $Session["users"]["message"].=', ' . $lang['cpanel']['users']['username_wrong']; 	
      $no = 1;
    }

    if (!$no) { //Si todo esta bien se inserta el user
	  $insertSQL = sprintf("INSERT INTO %susers (creation_date, update_date, username, id_group, name, lastname, email, homedir) VALUES (NOW(), NOW(), %s, %s, %s, %s, %s, %s);",
                           $tables_preffix,
      ...
      //Saco el ID del user que se inserto
	  $id_user = bfRecord(sprintf("SELECT id_user FROM %susers ORDER BY id_user DESC;", $tables_preffix));
      $id_user = $id_user->id_user;
	  //Mando a poner el password del user
...
<html>
<head>
<title><?php echo $lang['all']['bfexplorer']; ?> - <?php echo $lang['all']['control_panel']; ?> - <?php echo $lang['cpanel']['users']['new_user']; ?></title>
...
<div align="center"> 
  <table width="700" border="0" cellspacing="0" cellpadding="2"> 
    <tr height="30"> 
      ...
      <th width="99%" align="left" valign="middle" background="../../images/borders/topBorder.jpg"><?php echo $lang['all']['bfexplorer']; ?> - <?php echo $lang['all']['control_panel']; ?> - <?php echo $lang['cpanel']['users']['new_user']; ?></th> 
      <th width="4" background="../../images/borders/right_topCorner.jpg"></th> 
    </tr> 
    <tr> 
      <td background="../../images/borders/leftBorder.jpg"></td> 
      <td align="left" valign="top"><form action="new.php" method="POST" name="insert" id="insert" onSubmit="YY_checkform('insert','data[username]','#q','0','<?php echo $lang['cpanel']['users']['username_wrong']; ?>','data[name]','#q','0','<?php echo $lang['cpanel']['users']['name_wrong'] ?>','data[email]','#S','2','<?php echo $lang['cpanel']['users']['email_wrong']; ?>','data[id_group]','#q','1','<?php echo $lang['cpanel']['users']['group_wrong']; ?>');return document.MM_returnValue"> 
          <table width="100%" border="0" cellspacing="3" cellpadding="5" align="center"> 
            ...
            <tr> 
              ...
              <td width="66%"><input type="text" class="input" name="data[username]" id="data[username]"  value="<?php echo (isset($data["username"])) ? $data["username"] : ""; ?>"></td> 
            </tr> 
            <tr> 
            ...
            <tr align="right"> 
              ...
              <td><input type="hidden" name="accion" value="insert"> 
                ...
                <input name="Insert" type="submit" class="boton" id="Insert" value="<?php echo $lang['all']['insert']; ?>"></td> 
            </tr> 
          </table> 
        </form></td> 
